class FlowProcessor {
    ...
    processFlows(ref, flowSeries, leaking = true, reachable = true, className = this.templateClassName, methodSignature = this.templateMethodSignature) {
        flowSeries = Array.from(flowSeries);
        ...
        if (!!ref && ref.module !== 'empty') {
            ref.flows?.forEach(flow => {
                ...
                if (flowSeries.includes(String(ref.module))) {
        ...
        ref.children.forEach((child, idx) => {
            if (child.module !== 'empty') {
                this.processFlows(child, flowSeries, ref.flows[idx].leaking, ref.flows[idx].reachable, ref.flows[idx].className, ref.flows[idx].methodSignature);
            }
        });
    }

    getSourceSinkConnections(ref, flowSeries, connections = [], sources = [], idx = 0) {
        ...
        if (!!ref && ref.module !== 'empty') {
            ...
            ref.children.forEach((child, idx) => {
                this.getSourceSinkConnections(child, flowSeries, connections, currentSources, idx);
            });
    ...
    // full ground truth
    getAllConnections(ref, flowSeries, connections = [], parentFlow = null) {
        if (!!ref) {
            if (flowSeries.includes(String(ref.module))) {
                ...
                ref.children.forEach((child, idx) => {
                    let parentFlowNew = ref.flows[idx].statementSignature ? {
                        ...ref.flows[idx],
                        id: ref.id
                    } : parentFlow;
                    ...
                    this.getAllConnections(child, flowSeries, connections, parentFlowNew);
                });
            } else {
                ref.children.forEach((child, idx) => {
                    this.getAllConnections(child, flowSeries, connections, parentFlow);
                });
            }
        }
